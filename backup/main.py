...
def battle_en(board, x, y, characters, index):
    print(x, y)
    ...
    if is_defeated[0]:
        ...
        for key in range(40, 45):
            ...
            for enemy in range(len(board.enemies[f'{key}'])):
                ...
                if (is_defeated[-1] - delta) <= x_coord_enemies <= (is_defeated[-1] + delta) and \
                        (is_defeated[-2] - delta) <= y_coord_enemies <= (is_defeated[-2] + delta):
                    board.enemies[f'{key}'].pop(enemy)
                    board.items_map_for_current_level[board.current_room_y][board.current_room_x][y_coord_enemies][
                        x_coord_enemies] = '0'
                    break
                # index_enemy += 1
